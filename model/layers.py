...
class CoordConv(nn.Module):
    ...
    def add_coord(self, input):
        ...
        y, x = torch.meshgrid(y_range, x_range, indexing='ij') # Use 'ij' indexing for consistency
        y = y.expand([b, 1, -1, -1])
        x = x.expand([b, 1, -1, -1])
...
class Projector(nn.Module):
    def __init__(self, word_dim=1024, in_dim=512, kernel_size=3):
        super().__init__()
        self.in_dim = in_dim
        ...
        # textual projector
        out_dim = 1 * (in_dim // 2) * kernel_size * kernel_size + 1 
    ...
    def forward(self, x, word):
        ...
        '''
        x = self.vis(x)
        # After self.vis, x has in_dim // 2 channels
        B, C, H, W = x.size()
...
class GALayer(nn.Module):
    def __init__(self,
                 d_model=512,
                 nhead=8, # nhead should be 8, not 9
